...
int main()
{
    ...
    //Initialise the button pin with pull-up resistor
    ...
    while (true) {
        ...
        //Comment out old IMU Print line
        // printf("AX:%d AY:%d AZ:%d GX:%d GY:%d GZ:%d TEMP:%d\n",
        //     ax, ay, az, gx, gy, gz, temp);

        // Read the button state (active low, so invert it)
        int button_pressed = !gpio_get(BUTTON_PIN);
        // Send tilt and button state to Python
        printf("%d,%d\n", ay, button_pressed);

        ssd1306_clear();

        drawString(0, 0, "brick breaker");
